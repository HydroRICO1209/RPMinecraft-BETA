    ...
    async def open(self, ctx, *, arg: str):
        try:
            ...
            if arg in ['c', 'common chest', 'common']:
                lootlist = f'**{ctx.author.name}** opened a common chest and got:'
                if item.common_chest > 0: #have the chest
                    db[item.userid + 'common_chest'] -= 1
                    common = ['pogchop', 'coal']
                    for loot in common:
                    ...
                    for loot in super_rare:
                        if num3 == 69: #1% drop rate
                            my_emote = discord.utils.get(bot.emojis, name = loot)
                            db[f'{item.userid}{loot}'] += 1
                            lootlist += f'\n{my_emote}{loot} x1'

                    await ctx.send(lootlist)
                else:
                    await ctx.send(f'**{ctx.author.name}**, check your inventory maybe???')

            elif arg in ['r', 'rare chest']:
                lootlist = f'**{ctx.author.name}** opened a rare chest and got:'
                if item.rare_chest > 0: #have the chest
                    db[item.userid + 'rare_chest'] -= 1
                ...
                lootlist = f'**{ctx.author.name}** opened a super rare chest and got:'
                if item.super_rare_chest > 0: #have the chest
                    db[item.userid + 'super_rare_chest'] = item.super_rare_chest - 1
                ...
                lootlist = f'**{ctx.author.name}** opened an epic chest and got:'
                if item.epic_chest > 0: #have the chest
                    db[item.userid + 'epic_chest'] = item.epic_chest - 1
                ...
                if item.mythic_chest > 0: #have the chest
                    lootlist = f'**{ctx.author.name}** opened a mythic chest and got:'
                    db[item.userid + 'mythic_chest'] = item.mythic_chest - 1
                ...
                lootlist = f'**{ctx.author.name}** opened a legendary chest and got:'
                if item.legendary_chest > 0: #have the chest
                    db[item.userid + 'legendary_chest'] = item.legendary_chest - 1
                    ...
                    for _ in range(3):
                        ...
                        if loot in ['map_scrap', 'wither_skull', 'ender_pearl']:
                            db[f'{item.userid}{loot}'] += 2
                            lootlist += f'\n{my_emote}{loot} x2'
                        else:
                            db[f'{item.userid}{loot}'] += 20
                            lootlist += f'\n{my_emote}{loot} x20'
                ...
                await ctx.send(f'**{ctx.author.name}**, what are u even trying to open =.=')
